...
- Ensure you got good usb connection to external camera if using PC. Each loose connection to camera will disconnect your exam session. Then you have to re-log and wait for proctor to let you join the session again. Time continue to runs, and noway u get reimbursed
...
- Use [CTRL + Shift + C] and [CTRL + Shift + V] to copy and paste inside the terminal
...
##  The ADD command does not unpack zip files automatically
...
```
- Write the Dockerfile and then run podman build to produce the image. if <tag-name> not specified, it will use latest
```
...
```
- Upload image to an external image registry. tag flash is optional if you want to use latest tag
```
sudo podman [--creds] push <image-name>[:tag] <destination-url/path/image>[:tag]
sudo podman --creds $user:$password localhost/webserver:latest quay.io/jason.wong76/webserver:latest
```
